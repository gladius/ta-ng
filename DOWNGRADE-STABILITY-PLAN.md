...
- **`max_tokens` is an OUTPUT cap, not a context cap — keep it, size it right.** It bounds only what the profiler/judge *generate* (cost ~4–5× input, latency, no rambling); the reply size does NOT grow with input size (a judge comparing two million-token outputs still returns one KEPT/BROKE line). Setting it too *low* truncates the verdict → the "no verdict → broke" false positive we hit at 200/1000/320. It is a **CEILING, not a target** — the model stops the instant it's done, so the *judge* cap should be generous (~512, free insurance against the truncation cliff when it enumerates facts), while the *profiler* cap forces brevity (a bounded profile). Residual "no verdict" misses are caught by the SAFE fallback (→ NOT-SAFE), so they're conservative noise, never a false SAFE. The real enterprise-scale limit is the **INPUT** side: huge outputs vs `AUDIT_JUDGE_MAX_CHARS` + the model's context window. Today's `"too long → broke"` guard over-rejects big-but-legit outputs at scale → a **scale item** (larger-context judge, or structured field-level / diff comparison), independent of `max_tokens`.
...
- **All knobs live in `config.py`, `.env`-overridable** (matching the existing `AUDIT_*` block) — no magic numbers in the engine. Phase-1 additions: `AUDIT_PROFILER_MAX_TOKENS`, `AUDIT_JUDGE_MAX_TOKENS` (the judge OUTPUT cap — *currently hardcoded `200` in `judge_preserved`, a pre-existing gap to fix*), the profiled-flow K counts, the tolerance floor, the feature flag, `AUDIT_PROFILER_SAMPLE_CHARS`.
